...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fe = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=6, kernel_size=3, stride=1, padding=1),  # (a)
            nn.ReLU(),                                                                      # (b)
            nn.MaxPool2d(kernel_size=2, stride=2),                                         # (c)
            nn.Conv2d(in_channels=6, out_channels=6, kernel_size=3, stride=1, padding=1),  # (d)
            nn.ReLU(),                                                                      # (e)
            nn.MaxPool2d(kernel_size=2, stride=2),                                         # (f)
            nn.Flatten()                                                                    # (g)
        )
        self.fc = nn.Sequential(
            nn.Linear(in_features=6*7*7, out_features=100),  # (h)
            nn.ReLU(),                                        # (i)
            nn.Linear(in_features=100, out_features=10)       # (j)
        )
